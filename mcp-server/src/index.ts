  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "Lists available attribute options (hair/outfit/background) used for random variation.",
        inputSchema: {
          type: "object",
          properties: {},
        },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            randomize: {
              type: "boolean",
              description:
                "Whether to randomly vary unlocked attributes (hair style/outfit/background). Default true.",
              default: true,
            },
            locks: {
              type: "object",
              description:
                "Lock certain attributes. Default locks: face=true, hair_color=true (others unlocked).",
              properties: {
                face: { type: "boolean" },
            ...
            mood: {
              type: "string",
              description:
                "Optional mood/template override (backward-compatible). Prefer `template` when using presets.",
            },
            aspect_ratio: {
              type: "string",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (request.params.name === "generate_xhs_image") {
    ...
    try {
        const templates = await loadTemplates(EXTENSION_ROOT);
        const chosenName = (template ?? mood ?? templates.config.defaults?.template ?? "lifestyle") as string;
        ...
        if (typeof hair_style === "string" && hair_style.trim()) locks.hair_style = true;
        if (typeof hair_color === "string" && hair_color.trim()) locks.hair_color = true;
        if (typeof outfit === "string" && outfit.trim()) locks.outfit = true;
        if (typeof background === "string" && background.trim()) locks.background = true;

        const pickedHairStyle =
          typeof hair_style === "string" && hair_style.trim()
            ? hair_style.trim()
        ...
        const pickedOutfit =
          typeof outfit === "string" && outfit.trim()
            ? outfit.trim()
        ...
        if (locks.face) lockLines.push("Identity: locked (keep face/facial features consistent; no identity change).");
        if (locks.hair_color && !explicitHairColor) lockLines.push("Hair color: locked (keep consistent; do not change).");
        if (locks.hair_style && !pickedHairStyle) lockLines.push("Hair style: locked (keep consistent; do not change).");
        if (locks.outfit && !pickedOutfit) lockLines.push("Outfit: locked (keep consistent; do not change).");
        if (locks.background && !pickedBackground) lockLines.push("Background: locked (keep consistent; do not change).");

        const attributeLines: string[] = [];
        if (explicitHairColor) attributeLines.push(`Hair color: ${explicitHairColor}`);
        if (pickedHairStyle) attributeLines.push(`Hair style: ${pickedHairStyle}`);
        if (pickedOutfit) attributeLines.push(`Outfit: ${pickedOutfit}`);
        if (pickedBackground) attributeLines.push(`Background: ${pickedBackground}`);
